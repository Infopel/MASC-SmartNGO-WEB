...
<p>
    ...
    {{-- - --}}
Foi criada uma tarefa: (<a href="#">{{ $issue->subject ?? 'nome da tarefa'}}</a>) no projecto
<a href="{{ route('projects.overview', ['project_identifier' => $issue->project->identifier ?? 'project-not-found']) }}" target="_blank">{{ $issue->project->name ?? 'Project Not Found' }}</a> a decorrer ({{ $issue->start_date ?? 'dd/mm/yyyy' }})
pelo <i>Assistente de Projectos - {{ $issue->author->full_name ?? 'Author not found' }}</i>. Para avaliação e aprovação da tarefa <a href="{{ route('orcamento.projecto.solicitacao-fundos.show', ['project_identifier' => $issue->project->identifier ?? 'project-not-found', 'issue' => $issue->id ?? 9999] ) }}" target="_blank">clique aqui</a>.
</p>
  {{-- - --}}
<br>
